...
outliers_data$Status <- ifelse(outliers, "Outlier", "Normal")
outliers_data <- outliers_data[, c(1, ncol(outliers_data), 2:(ncol(outliers_data)-1))]
...
# Run the Shiny app
  brushed_data <- reactive({
    brushedPoints(outliers_data, input$brush, xvar = input$xvar, yvar = "BODYFAT")
  })
